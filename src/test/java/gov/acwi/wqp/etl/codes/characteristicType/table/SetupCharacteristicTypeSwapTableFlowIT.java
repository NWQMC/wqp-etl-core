...
public class SetupCharacteristicTypeSwapTableFlowIT extends BaseFlowIT {
	...
	public static final String EXPECTED_DATABASE_QUERY = "select table_catalog, table_schema, table_name, table_type from information_schema.tables where table_name='char_type_swap_stewards'";
	...
	public void dropCharacteristicTypeSwapTableStepTest() {
		try {
			JobExecution jobExecution = jobLauncherTestUtils
					.launchStep("dropCharacteristicTypeSwapTableStep", testJobParameters);
	...
	public void createCharacteristicTypeSwapTableStepTest() {
		try {
			JobExecution jobExecution = jobLauncherTestUtils
					.launchStep("createCharacteristicTypeSwapTableStep", testJobParameters);
			assertEquals(ExitStatus.COMPLETED, jobExecution.getExitStatus());
		} catch (Exception e) {
			e.printStackTrace();
			fail(e.getLocalizedMessage());
